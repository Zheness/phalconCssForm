...
class phalconCSSFormPureCSS extends \Phalcon\Forms\Form {
    ...
    public function renderForm() {
        ...
        $elementClasses = "pure-form pure-form-stacked ";
    ...
    function _renderDefault($element) {
        ...
        $html = "<p><label for=\"{$element->getName()}\">{$element->getLabel()}</label>";
    ...
    function _renderCheck($element) {
        ...
        $html = "<p><label for=\"{$element->getName()}\">{$this->render($element->getName())} {$element->getLabel()}</label></p>";
        if ($hasMessages)
            $html .= "<p>{$messages[0]}</p>";
        return $html;
    }
    ...
    function _renderSubmit($element) {
        $elementClasses = "pure-button " . $element->getAttribute("class", "");
