...
class phalconCSSFormUIkit extends \Phalcon\Forms\Form {
    ...
    public function renderForm() {
        ...
        $elementClasses = "uk-form uk-form-stacked ";
        if (isset($this->_formAttributes["class"])) {
            $elementClasses .= $this->_formAttributes["class"];
            unset($this->_formAttributes["class"]);
        }
    ...
    function _renderDefault($element) {
        ...
        $divError = $hasMessages ? "uk-form-danger" : "";
        $html = "<div class=\"uk-form-row\">";
        $html .= "<label class=\"uk-form-label\" for=\"{$element->getName()}\">{$element->getLabel()}</label>";
        $elementClasses = "uk-form-controls {$divError} " . $element->getAttribute("class", "");
        $element->setAttribute("class", $elementClasses);
        $html .= $this->render($element->getName());
        if ($hasMessages)
            $html .= "<p class=\"uk-form-help-block\">{$messages[0]}</p>";
        $html .= "</div>";
        return $html;
    }
    ...
    function _renderSubmit($element) {
        $elementClasses = "uk-button " . $element->getAttribute("class", "");
        $element->setAttribute("class", $elementClasses);
        $html = $this->render($element->getName());
        return $html;
    }
